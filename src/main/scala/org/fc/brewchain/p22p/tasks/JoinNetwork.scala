...
import org.fc.brewchain.p22p.pbgens.P22P.PMJoinedNode
...
case class JoinNetwork(network: Network, statupNodes: Iterable[PNode]) extends SRunner with PMNodeHelper with LogHelper {
  ...
  def syncDB() {
    ...
    Daos.odb.put(OKey.newBuilder().setData(ByteString.copyFrom(DB_KEY)).build(),
      OValue.newBuilder().setExtdata(pmb.build().toByteString()).build())
  }
  def init() {
    loadFromDB()
  }
  def loadFromDB() {
    val v = Daos.odb.get(OKey.newBuilder().setData(ByteString.copyFrom(DB_KEY)).build())
    if (v == null || v.get() == null) {
      log.debug("none block list or joined nodes");
    } else {
      val pmb = PMJoinedNode.newBuilder().mergeFrom(v.get.getExtdata)
