...
case class VoteQueue(network: Network) extends LogHelper {
  ...
  val inQ = new LinkedBlockingQueue[(PVBase, Array[Byte], PBFTStage)](); //,new LinkedBlockingQueue[(Network,PVBase, OValue.Builder, PBFTStage)]]();
  ...
  def pollQ(): (PVBase, Array[Byte], PBFTStage) = {
