...
import org.fok.p22p.stat.MessageCounter
import org.apache.commons.lang3.StringUtils
import java.util.HashMap
import java.util.concurrent.ConcurrentHashMap
import org.fok.p22p.stat.MessageCounter.CCSet
import org.fok.p22p.node.router.RandomNR
...
object PNode {
  ...
  def signNode(name: String, node_idx: Int, //node info
    ...
    if (StringUtils.isNotBlank(pri_key)) {
      PNode(name, node_idx, Daos.enc.bytesToHexStr(Daos.enc.sign(Daos.enc.hexStrToBytes(pri_key), Array(bcuid, v_address).mkString("|").getBytes)),
        uri, //
        startup_time, //
        pub_key, //
        counter,
...
case class ClusterNode(net_id: String, root_name: String, cnode_idx: Int, //node info
  ...
  def signNode(): ClusterNode =
    ClusterNode(
      ...
      if (StringUtils.isNotBlank(pri_key)) {
        Daos.enc.bytesToHexStr(Daos.enc.sign(Daos.enc.hexStrToBytes(pri_key), Array(bcuid, v_address).mkString("|").getBytes)) //
      } else {
        sign
      }, pnodes, counter,
      startup_time, //
  ...
  override def changeIdx(idx: Int): Node = {
    ClusterNode(
      ...
      if (StringUtils.isNotBlank(pri_key)) {
        Daos.enc.bytesToHexStr(Daos.enc.sign(Daos.enc.hexStrToBytes(pri_key), Array(idx, bcuid, v_address).mkString("|").getBytes)) //
      } else {
        sign
      }, pnodes, counter,
      startup_time, //
  ...
  override def changeVaddr(vaddr: String): Node = {
    ClusterNode(
      net_id, root_name, node_idx, //node info
      if (pri_key != null) {
        Daos.enc.bytesToHexStr(Daos.enc.sign(Daos.enc.hexStrToBytes(pri_key), Array(node_idx, bcuid, vaddr).mkString("|").getBytes)) //
      } else {
        sign
      }, pnodes, counter, //
      startup_time, //
  ...
  override def changeName(name: String): Node = {
    ClusterNode(
      ...
      if (StringUtils.isNotEmpty(pri_key)) {
        Daos.enc.bytesToHexStr(Daos.enc.sign(Daos.enc.hexStrToBytes(pri_key), Array(node_idx, bcuid, v_address).mkString("|").getBytes)) //
      } else {
        sign
      }, pnodes, counter, //
      startup_time, //
